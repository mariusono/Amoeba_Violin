...
void MainComponent::prepareToPlay(int samplesPerBlockExpected, double sampleRate) // how do I change the sampleRate .. ?
{
    ...
    File resourceFile("C:\\Marius\\Amoeba_Violin_DynFr\\Amoeba_Violin_DynFr\\x_locs_Matlab_N21.txt");
    ...
    File resourceFile_Y("C:\\Marius\\Amoeba_Violin_DynFr\\Amoeba_Violin_DynFr\\y_locs_Matlab_N21.txt");
    ...
    I_P[l_inp_plate + m_inp_plate * N_plate] = (1 - alpha_x_inp) * (1 - alpha_y_inp); // l_inp*N+m_inp is equivalent to array[l_inp][m_inp] if it would be 2D
    I_P[l_inp_plate_plus1 + m_inp_plate * N_plate] = alpha_x_inp * (1 - alpha_y_inp);
    I_P[l_inp_plate + m_inp_plate_plus1 * N_plate] = (1 - alpha_x_inp) * alpha_y_inp;
    I_P[l_inp_plate_plus1 + m_inp_plate_plus1 * N_plate] = alpha_x_inp * alpha_y_inp;
...
void MainComponent::getNextAudioBlock(const juce::AudioSourceChannelInfo& bufferToFill)
{
    ...
    for (int channel = 0; channel < bufferToFill.buffer->getNumChannels(); ++channel)
    {
        ...
        for (int i = 0; i < bufferToFill.numSamples; i++)
        {
            ...
            if (channel == 0) //// this condition causes the output to explode sometimes
            {
                ...
                //              //float val = std::accumulate(u_times_Igrid.begin(), u_times_Igrid.end(), 0);
                //              //Logger::getCurrentLogger()->outputDebugString("val: (" + juce::String(val) + ")");

                //              resultant = sqrt(x_var * x_var + y_var * y_var);
                ...
                //} //// this condition causes the output to explode sometimes
                //else
                //{
                //    Fbow = 0;
                ...
                //Logger::getCurrentLogger()->outputDebugString("FB_var: (" + String(FB_var) + ")");
  ...
  //              //}
        ...
        //outputR[i] = outputL[i];
            ...
            //double rate = 0.81;
            //double newLength = floor(bufferToFill.numSamples / rate);
            //double a = 0;
            //double b = static_cast <double> (bufferToFill.numSamples);
            //std::vector<double> n_new_interp = linspace <double>(a, b, newLength);
            //std::vector<int> integ;
            //integ.resize(n_new_interp.size(),0);
            ////std::vector<double> fract;
            ////fract.resize(0, n_new_interp.size());
            //int annna = 5;

            //std::vector<double> data_new;
            //data_new.resize(n_new_interp.size(), 0);



            //for (int iTest = 0; iTest < n_new_interp.size(); iTest++)
            //{
            //    integ[iTest] = floor(n_new_interp[iTest]);
            //    //fract[i] = n_new_interp[i] - integ[i];
            //}

            ////std::vector<int> check = unique_ids <int>(integ);

            //std::vector<int> unqVals = unique_vals <int>(integ);

            //// this tries to replicate the matlab unique function..
            //std::vector<int> check = unique_ids2 <int>(integ, unqVals);
            //std::vector<int> check2 = unique_ids2 <int>(integ,check);
            //std::vector<int> check3 = unique_vals <int>(check2);

            //for (int iterVar = 0; iterVar < output_interim.size(); iterVar++)
            //{
            //    data_new[check3[iterVar]] = output_interim[iterVar];
            //}

            //int anaaaaa = 1;

            ////for (int iterVar = 0; iterVar < bufferToFill.numSamples; iterVar++)
            ////{
            ////    outputL[iterVar] = data_new[iterVar];
            ////    outputR[iterVar] = outputL[iterVar];
            ////}

            //for (int iterVar = 0; iterVar < bufferToFill.numSamples; iterVar++)
            //{
            //    outputL[iterVar] = output_interim[iterVar];
            //    outputR[iterVar] = outputL[iterVar];
            //}
    ...
    for (auto sensel : sensels)
    {
        ...
        if (sensel->senselDetected)
        {
            ...
            for (int f = 0; f < fingerCount; f++)
            {
                ...
                //Logger::getCurrentLogger()->outputDebugString("x_bow_perc: (" + String(x_bow_perc) + ")");
                //Logger::getCurrentLogger()->outputDebugString("y_bow_perc: (" + String(y_bow_perc) + ")");
                //Logger::getCurrentLogger()->outputDebugString("vB_var: (" + String(vB_var) + ")");
                //Logger::getCurrentLogger()->outputDebugString("fN_var: (" + String(fN_var) + ")");
